...
router.post('/', authenticateToken, (req, res) => {
  ...
  const { code, name, type, parent_id, currencies } = req.body;
  ...
  const insertAccount = db.transaction(() => {
    const result = db.prepare('INSERT INTO accounts (code, name, type, parent_id) VALUES (?, ?, ?, ?)')
      .run(code, name, type, parent_id || null);
...
router.put('/:id', authenticateToken, (req, res) => {
  ...
  const { code, name, type, parent_id, is_active, currencies } = req.body;
  const { id } = req.params;
  ...
  const updateAccount = db.transaction(() => {
    db.prepare('UPDATE accounts SET code = ?, name = ?, type = ?, parent_id = ?, is_active = ? WHERE id = ?')
      .run(code, name, type, parent_id || null, is_active !== undefined ? is_active : 1, id);
