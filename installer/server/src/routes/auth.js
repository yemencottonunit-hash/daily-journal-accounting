...
router.post('/login', (req, res) => {
  try {
    ...
    res.json({
      token,
      user: { id: user.id, username: user.username, full_name: user.full_name, role: user.role, avatar: user.avatar || '' }
    });
  } catch (err) {
...
router.get('/users', authenticateToken, (req, res) => {
  ...
  const users = db.prepare('SELECT id, username, full_name, role, avatar, is_active, created_at FROM users ORDER BY id').all();
  res.json(users);
});
...
router.post('/users', authenticateToken, (req, res) => {
  ...
  db.exec(`INSERT INTO users (username, password, full_name, role) VALUES ('${username.replace(/'/g, "''")}', '${hashedPassword.replace(/'/g, "''")}', '${(full_name || '').replace(/'/g, "''")}', '${(role || 'accountant').replace(/'/g, "''")}')`);
...
router.put('/users/:id', authenticateToken, (req, res) => {
  ...
  db.exec(`UPDATE users SET full_name = '${(full_name || '').replace(/'/g, "''")}', role = '${(role || 'accountant').replace(/'/g, "''")}', is_active = ${is_active !== undefined ? (is_active ? 1 : 0) : 1} WHERE id = ${id}`);
